...
There is currently a migration of OneDrive underway to a new platform, the way to extract OD authentication token seems to be broken. For the time being, copy a working auth token from web browser to the script (it works for about an hour, so it should be enough to download your photos).
...
* -AccessToken - authentication token to use, "stolen" from onedrive website communication via Dev Tools window
